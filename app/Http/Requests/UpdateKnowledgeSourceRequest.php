...
class UpdateKnowledgeSourceRequest extends FormRequest
{
    ...
    {
        $knowledgeSource = $this->route('knowledgeSource');
        ...
        if ($this->input('type') === 'codebase') {
            ...
            // Private key is optional on update (keep existing if blank)
            $rules['ssh_private_key'] = 'nullable|string';
        }
    ...
    {
        $knowledgeSource = $this->route('knowledgeSource');
        $existingConfig = $knowledgeSource->config ?? [];
        ...
        if ($this->input('type') === 'codebase') {
            return [
                ...
                'ssh_private_key' => $this->input('ssh_private_key')
                    ?: ($existingConfig['ssh_private_key'] ?? ''),
            ];
        }
